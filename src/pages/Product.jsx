    ...
    <section className="py-24">
      <div className="container max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 px-4 md:px-0">
        ...
        <div className="text-center lg:text-left">
          ...
          {/* Rating */}
          <div className="mt-4 flex items-center sm:justify-center lg:justify-start gap-2">
          ...
          <div className="mt-6 md:mt-8">
            <h3 className="font-semibold mb-3">{t("product.fragranceNotes")}</h3>
            <div className="flex flex-wrap gap-2 md:gap-3 justify-center lg:justify-start">
              {product.notesKeys.map((note, i) => (
                <span
                  key={i}
                  className="px-3 md:px-4 py-1.5 md:py-2 bg-[var(--bg-soft)] rounded-full text-xs sm:text-sm"
                >
                  {t(`products.notes.${note}`) || note} {/* Если перевода нет, покажет оригинал */}
                </span>
              ))}
            </div>
          </div>
      ...
      <div className="container max-w-6xl mx-auto mt-16 md:mt-24 mb-12 px-4 md:px-0">
        <h2 className="text-2xl font-serif mb-12 text-center">{t("product.reviews")}</h2>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          {product.commentsKeys.map((cId) => {
      ...
      {relatedProducts.length > 0 && (
        <div className="container max-w-6xl mx-auto mt-16 md:mt-24 px-4 md:px-0 mb-8">
          <h2 className="text-2xl font-serif mb-12 text-center">{t("product.relatedProducts")}</h2>
